...
AI 응용 기술을 활용한 YouTube 학습 코스 공유 게시판입니다. 사용자는 좋은 YouTube 강의를 영상 보드에 등록하고, 각 영상의 요약과 전사문 지식을 저장한 뒤, 여러 영상을 순서 있는 학습 코스로 묶어 다른 사람과 공유하며 완주 피드백을 남길 수 있습니다. AI 기능은 과제 요구사항에 맞춰 RAG, MCP, AI Agent를 각각 분리해 구현했습니다.
...
기능명: 기존 코스 우선 탐색을 위한 영상 전사문 RAG

흐름:

1. 사용자가 코스 찾기 화면에서 원하는 학습 코스를 입력합니다.
2. 각 영상 카드는 `summary`와 `translatedNotes`에 요약 및 전사문 지식을 저장합니다.
...
- `retrieve_posts`: RAG로 영상 요약과 전사문 지식 검색
...
- `/board`에서 URL 붙여넣기 → MCP 자동 입력 → 요약/전사문 지식 저장 → Agent 코스 조합 과정을 확인
- 중앙 영상 요약, 추천 난이도, 대상 사용자, RAG 전사문 지식, 댓글
- 하단 영상 지식 카드 작성/수정/삭제
- 영상 상세에서 영상을 바로 학습 코스 재료로 담고 내부 플레이어로 이동
- `/playlists`의 코스 찾기 화면에서 RAG가 기존 코스/영상 보드를 먼저 검색
- 원하는 코스가 이미 있으면 기존 코스와 전사문 근거 영상을 먼저 안내
...
이번 프로젝트는 단순 게시판에 AI 기능을 덧붙이는 방식이 아니라, 영상 카드마다 요약과 전사문 지식을 저장하고 RAG가 그 근거 조각을 검색하도록 설계했습니다. MCP는 영상 메타데이터를 채우고, Agent는 전사문 RAG 근거와 외부 YouTube 후보를 조합해 공유 가능한 학습 코스를 만듭니다. 프론트엔드, NestJS API, FastAPI AI 서비스를 분리하면서 실제 서비스형 아키텍처를 경험할 수 있습니다.
...
- YouTube transcript API를 더 안정적으로 연동해 실제 전사문 수집과 번역을 자동 생성합니다.
