...
AI 응용 기술을 활용해 각 팀원이 서로 다른 도메인의 게시판 서비스를 구현한 포트폴리오 저장소입니다. 공통 과제는 단순 CRUD 게시판을 넘어 인증, 검색, 댓글, 파일/외부 데이터, RAG, MCP, Agent 흐름을 실제 서비스 구조로 연결하는 것이었습니다.
...
| --- | --- | --- | --- |
| `ck/` | 뮤지컬 좌석 후기 게시판 | 공연장 좌석 리뷰, 시야/음향 평가 | 좌석 단위 후기, 태그/댓글/신고/관리자, RAG 질의, 좌석 추천 Agent |
...
프로젝트마다 ORM과 세부 라이브러리는 다릅니다. 예를 들어 `ck`와 `gg`는 Prisma 기반 구성이 있고, `jh`는 TypeORM 기반 백엔드 구성을 사용합니다. `siwon`은 PostgreSQL + pgvector와 파일 기반 fallback 저장소를 함께 둔 구조입니다.
...
공통적으로 프론트엔드는 NestJS API만 호출하고, AI 서버와 외부 API 호출은 백엔드 또는 AI 서버 뒤로 숨겼습니다. 이 구조는 API Key 노출을 막고, 인증/권한/데이터 조회를 NestJS에서 일관되게 처리하기 위한 선택입니다.

## 개인 구현 요약

### `ck/` 뮤지컬 좌석 후기 게시판

뮤지컬 관람 후기를 좌석 단위로 축적하고, 사용자가 좌석 선택 전에 시야와 음향 정보를 비교할 수 있게 만든 서비스입니다.

- 극장, 공연, 층, 구역, 열, 번호 기반 좌석 후기 작성
- 시야, 음향, 편의성, 표정, 무대 가시성 등 좌석 경험 평가
...
일반 커뮤니티 게시판 기능에 반려동물 행동 Q&A와 장소 탐색 기능을 결합한 서비스입니다.

- 회원가입, 로그인, 게시글 CRUD
- 댓글, 태그/카테고리, 좋아요, 검색, 페이징
...
각 프로젝트는 독립적인 의존성, 환경 변수, DB 설정을 갖습니다. 실행 전에는 해당 디렉터리의 README 또는 `package.json` 스크립트를 먼저 확인하세요.
...
- 인증, 권한, 검색, 댓글, 좋아요, 파일, 관리자 기능처럼 실제 서비스에 필요한 주변 기능을 포함했습니다.
- AI 기능을 화면에 붙이는 데서 끝내지 않고, 백엔드 데이터 조회와 AI 서버 도구 호출을 분리했습니다.
...
- 하위 프로젝트의 세부 기능, API, 테스트 방법은 각 디렉터리 README와 문서를 기준으로 확인합니다.
