...
@Controller()
export class LegacyEmailAuthController {
  constructor(
  ...
  async registrationReadiness(@Req() request: Request) {
    ...
    if (!enrollmentToken) throw this.invalidEnrollment();
    const result =
      await this.authService.getRegistrationReadiness(enrollmentToken);
    if (result.status !== 'ready') throw this.invalidEnrollment();
  ...
  async completeRegistration(
    ...
    if (result.status !== 'completed') throw this.invalidEnrollment();
